...
const createRequestTable = ';DROP TABLE IF EXISTS requests CASCADE; CREATE TABLE requests(requestId SERIAL PRIMARY KEY NOT NULL, firstName VARCHAR(100), lastName VARCHAR(100), department VARCHAR(100), userId SERIAL NOT NULL, equipment VARCHAR(100) NOT NULL,  description TEXT NOT NULL, status VARCHAR(100), date DATE DEFAULT CURRENT_DATE)';
...
const insertRequestTable = ';INSERT INTO requests(userId, firstName, lastName, department, equipment, description, status) VALUES(1, \'fayokemi\', \'adeyina\', \'organizing\', \'generator\', \'Generator goes off always\', \'pending\')';
const insertRequest2Table = ';INSERT INTO requests(userId, firstName, lastName, department, equipment, description, status) VALUES(2, \'omotola\', \'adeyina\', \'medical\', \'Air Condition\', \'air condition blows hot air\', \'resolved\')';
